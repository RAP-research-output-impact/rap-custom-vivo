    ...
    this.datamap.svg.selectAll('.datamaps-bubble')
        .each(function (d) {
            if (d3.select(this).attr('ro') > 0) {
                d3.select(this).attr('r', d3.select(this).attr('ro') / scale);
            } else {
...
Zoom.prototype._getScalesArray = function() {
    ...
    for (var i = 0; i <= 5; i++) {
        array.push(Math.pow(Math.E, 0.2 * i * scaleMaxLog));
    }
    return array;
};
...
function Datamapy(data) {
    this.container = $("#copub-map");
...
Datamapy.prototype._handleMapReady = function(datamap) {
    this.zoom = new Zoom({
