...
//  ??? is this actually used?
    ...
    function fetchSubjectData() {
        var mapData = serviceBase + "copub-subjects?dept=" + $("#dtu-dep").val() + "&startYear=" + $('#year-from').val() + "&endYear=" + $('#year-to').val();
    ...
    function orgList(data) {
        ...
        for (var i = 0, j = data.subjects.length; i < j; i++){
            tbody += "<tr><td class=\"map-org-org sort-org\"><a href=\"" + profileBase + data.subjects[i].subject + "\">" + data.subjects[i].name +
                     "</a></td><td class=\"sort-pub\" style=\"text-align: right;\">" + data.subjects[i].publications + "</td></tr>";
        }
